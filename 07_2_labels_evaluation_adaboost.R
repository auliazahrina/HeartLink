...
for (i in 1:10)
{
  ...
  #Train data
  set.seed(3)
  adaboost <- adaboost(class~.,data=data_all.train,150)
  prediction <- predict(adaboost, newdata=data_all.test, type="class")
  table <- table(prediction$class, data_all.test$class)
  table
  ...
  roc_obj <- roc(data_all.test$class, as.numeric(prediction$class))
